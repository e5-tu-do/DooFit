...
if [ "$1" = "debug" ]
then
  $CMAKE_EXECUTABLE -DCMAKE_INSTALL_PREFIX=$DOOFITSYS -DCMAKE_BUILD_TYPE=Dbg $CMAKE_BASE_DIR
elif [ "$1" = "profiling" ]
then
  $CMAKE_EXECUTABLE -DCMAKE_INSTALL_PREFIX=$DOOFITSYS -DCMAKE_BUILD_TYPE=Profiling $CMAKE_BASE_DIR
else 
  $CMAKE_EXECUTABLE -DCMAKE_INSTALL_PREFIX=$DOOFITSYS -DCMAKE_BUILD_TYPE=Release $CMAKE_BASE_DIR
fi
