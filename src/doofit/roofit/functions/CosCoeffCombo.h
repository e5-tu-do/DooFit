...
#ifndef DOOFIT_ROOFIT_FUNCTIONS_COSCOEFFCOMBO
#define DOOFIT_ROOFIT_FUNCTIONS_COSCOEFFCOMBO
...
class CosCoeffCombo : public RooAbsReal {
public:
  
  CosCoeffCombo() {};
  
  CosCoeffCombo(std::string name,
                RooAbsReal& _par_C,
  ...
  CosCoeffCombo(const CosCoeffCombo& other, const char* name=0) ;
  virtual TObject* clone(const char* newname) const { return new CosCoeffCombo(*this,newname); }
  inline virtual ~CosCoeffCombo() {
    #ifdef FUNCTIONS_COUNT_CALLS
    std::cout << "CosCoeffCombo::~CosCoeffCombo(): # evaluate calls: " << num_calls_evaluate_ << std::endl;
    std::cout << "CosCoeffCombo::~CosCoeffCombo(): # integral calls: " << num_calls_integral_ << std::endl;
    #endif
  }
...
//    ++n_calls;
    ...
    return (tag*(1.0 - omega_Bd - omega_Bdb) - par_prod_asym*(1.0 - tag*(omega_Bd - omega_Bdb)))*par_C ;
  }
  ...
  //   #endif
    ...
    //assert(0 != code);
    // if (1 == code) {
      //std::cout << "CosCoeffCombo::analyticalIntegral(" << code << ", ...): Called." << std::endl;
      // return 0.0;
    // }
    // must not get here
    //assert(1 == 0);
...
private:
  ...
  ClassDef(CosCoeffCombo,1) // Your description goes here...
};
} // namespace functions
} // namespace roofit
} // namespace doofit
