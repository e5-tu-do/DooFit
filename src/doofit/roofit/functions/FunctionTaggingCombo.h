...
#ifndef DOOFIT_ROOFIT_FUNCTIONS_FUNCTIONTAGGINGCOMBO
#define DOOFIT_ROOFIT_FUNCTIONS_FUNCTIONTAGGINGCOMBO
...
class FunctionTaggingCombo : public RooAbsReal {
public:
  
  FunctionTaggingCombo() {};
  
  FunctionTaggingCombo(const char *name, const char *title,
  ...
  FunctionTaggingCombo(const FunctionTaggingCombo& other, const char* name=0) ;
  virtual TObject* clone(const char* newname) const { return new FunctionTaggingCombo(*this,newname); }
  inline virtual ~FunctionTaggingCombo() {
    #ifdef FUNCTIONS_COUNT_CALLS
    std::cout << "FunctionTaggingCombo::~FunctionTaggingCombo(): # evaluate calls: " << num_calls_evaluate_ << std::endl;
    std::cout << "FunctionTaggingCombo::~FunctionTaggingCombo(): # integral calls: " << num_calls_integral_ << std::endl;
    #endif
  }
...
protected:
  ...
  inline Double_t evaluate() const {
    ...
    //std::cout <<  << std::endl;
      ...
      if (cat_tag_OS == cat_tag_SS) return cat_tag_OS;
      else {
        if (omega_OS >= omega_SS) return cat_tag_SS;
        else return cat_tag_OS;
      }
  }
  ...
  virtual Double_t analyticalIntegral(Int_t code, const char* rangeName = 0) const {
    ...
    if (1 == code) {
      //std::cout << "FunctionTaggingCombo::analyticalIntegral(" << code << ", ...): Called." << std::endl;
      return 0.0;
    }
    // must not get here
    //assert(1 == 0);
...
private:
  ...
  ClassDef(FunctionTaggingCombo,1) // Your description goes here...
};
} // namespace functions
} // namespace roofit
} // namespace doofit
