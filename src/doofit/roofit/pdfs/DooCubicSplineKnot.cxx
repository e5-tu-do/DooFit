...
namespace DooCubicSplineKnot_aux {

  Double_t get(const RooArgList& b,int i) { return ((RooAbsReal&)b[i]).getVal() ; }
  Double_t get(const RooArgList& b,int i,int k) { return DooCubicSplineKnot_aux::get(b,i+k); }
...
void DooCubicSplineKnot::smooth(std::vector<double>& y, const std::vector<double>& dy, double lambda) const {
...
// see http://r.789695.n4.nabble.com/file/n905996/SPLINES.PDF
    ...
    if (lambda<0|| !(lambda<1)) {
        throw std::string("DooCubicSplineKnot::smooth: smoothing parameter must be in range [0,1)");
    }
...
void DooCubicSplineKnot::computeCoefficients(std::vector<double>& y, BoundaryConditions bc) const
{
...
void DooCubicSplineKnot::fillPQRS() const {
    assert(_PQRS.empty());
...
double DooCubicSplineKnot::evaluate(double x, const RooArgList& b) const {
    if ( (_use_range) && ( (x < _range_min) || (x > _range_max) ) ){ return 0; }
    using DooCubicSplineKnot_aux::get; 
...
double DooCubicSplineKnot::analyticalIntegral(const RooArgList& b) const {
    using DooCubicSplineKnot_aux::push_back;
    using DooCubicSplineKnot_aux::get;
    if (_IABCD.empty()) {
...
DooCubicSplineKnot::S_jk DooCubicSplineKnot::S_jk_sum(int i, const RooArgList& b) const
{
    if (_S_jk.empty()) {
        _S_jk.reserve(size()*4);
        for(int i=0;i<size();++i) {
            // This 'table' should be compatible with the definitions of A,B,C, and D...
            _S_jk.push_back( -DooCubicSplineKnot::S_jk(u(i+1),u(i+1),u(i+1))/P(i) ); // A
            _S_jk.push_back(  DooCubicSplineKnot::S_jk(u(i-2),u(i+1),u(i+1))/P(i)
                             +DooCubicSplineKnot::S_jk(u(i-1),u(i+1),u(i+2))/Q(i)
                             +DooCubicSplineKnot::S_jk(u(i  ),u(i+2),u(i+2))/R(i) ); // B
            _S_jk.push_back( -DooCubicSplineKnot::S_jk(u(i-1),u(i-1),u(i+1))/Q(i)
                             -DooCubicSplineKnot::S_jk(u(i-1),u(i  ),u(i+2))/R(i)
                             -DooCubicSplineKnot::S_jk(u(i  ),u(i  ),u(i+3))/S(i) ); // C
            _S_jk.push_back(  DooCubicSplineKnot::S_jk(u(i  ),u(i  ),u(i  ))/S(i) ); // D
        }
    }
    using DooCubicSplineKnot_aux::get;
...
DooCubicSplineKnot::S_edge DooCubicSplineKnot::S_jk_edge(bool left, const RooArgList& b) const {
       using DooCubicSplineKnot_aux::get;
       if (left) {
...
double  DooCubicSplineKnot::expIntegral(const TH1* hist, double gamma, TVectorD& coefficients, TMatrixD& covarianceMatrix) const {
    assert(hist!=0);
    int nknots = size();
