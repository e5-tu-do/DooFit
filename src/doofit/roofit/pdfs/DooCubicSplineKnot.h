#ifndef DOOFIT_ROOFIT_PDFS_CUBICSPLINEKNOT
#define DOOFIT_ROOFIT_PDFS_CUBICSPLINEKNOT
#include <vector>
#include <algorithm>
...
// this class is a slightly modified copy of RooCubicSplineKnot

// CINT doesn't like it if we encapsulate this in DooCubicSplineKnot ;-(

namespace doofit {
namespace roofit {
namespace pdfs {

class DooCubicSplineKnot {
public:
    ...
    DooCubicSplineKnot(const double *array, int nEntries ) : _u( array, array+nEntries),_use_range(false) {}
    template <typename Iter> DooCubicSplineKnot(Iter begin, Iter end) : _u(begin,end),_use_range(false) {}
    DooCubicSplineKnot(const std::vector<double>& knots ) // needed to easily interface with python...
        : _u(knots),_use_range(false) {}
    ...
    DooCubicSplineKnot::S_jk S_jk_sum(int i, const RooArgList& b) const ;
    ...
    DooCubicSplineKnot::S_edge S_jk_edge(bool left, const RooArgList& b) const;
...
private:
    ...
    mutable std::vector<DooCubicSplineKnot::S_jk> _S_jk; //!
