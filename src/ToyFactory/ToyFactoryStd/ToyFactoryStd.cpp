...
RooDataSet* ToyFactoryStd::Generate() {
  ...
  //RooArgSet* generated = data->get();
  //RooArgSet* to_generate = config_toyfactory_.argset_generation_observables();
  ...
  for (std::vector<DiscreteProbabilityDistribution>::const_iterator it = discrete_probabilities.begin(); it != discrete_probabilities.end(); ++it) {
    ...
    if (disc_var == NULL) {
      ...
      if (data->get()->contains(*disc_var)) {
        ...
        sinfo << "Generating discrete dataset for " << (*it).var_name() << endmsg;
        
        const std::map<double,std::pair<double,double> >& prob_map = (*it).probabilities_map();
        
        for (std::map<double,std::pair<double,double> >::const_iterator it_prob = prob_map.begin(); it_prob != prob_map.end(); ++it_prob) {
          sdebug << (*it_prob).second.second << endmsg;
        }
      }
    }
  }
